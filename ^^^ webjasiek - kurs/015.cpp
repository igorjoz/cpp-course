...
void _015() {
	...
	cout << "W¹tek g³ówny=" << this_thread::get_id() << endl;
		...
		// 1) Funkcja letters ruszy jako oddzielny w¹tek, jednak decyzjê o starcie pozostawiam maszynie a zatem w¹tek mo¿e w ogóle nie ruszyæ (a¿ do wywo³ania jawnie metody .get()).	
		std::future<int> result1(std::async(launch::async,letters, 65));
		/// uruchamia ale program idzie dalej (nie stoi w tym miejscu)
		/* Uwaga! Jeœli wywo³anie asynchroniczne nie jest mo¿liwe w tym miejscu, program zg³osi wyj¹tek std::system_error. Je¿eli uda³o siê uruchomiæ, wynik dzia³ania w¹tku zostanie przypisany specjalnemu obietowi result_1 klasy future<int>
		*/
		/*
	...
	/// UWAGA! w ostatecznoœci to, czy coœ "zyskamy" na wielow¹tkowoœci zale¿y od wielu czynników, choæby od zu¿ycia zasobów, obci¹¿eniu systemu, jakoœci tego, coœmy sp³odzili itp. itd.
	/// Ró¿ne w¹tki mog¹ mieæ dostêp do tych samych danych w pamiêci, co mo¿e prowadziæ do nieprzewidzianych skutków. Programowanie wielow¹tkowe jest pozornie proste, w praktyce musi byæ bardzo dobrze zaprojektowane, aby w¹tki faktycznie wykorzystywa³y moce procesorów i aby nie kolidowa³y, tworz¹c kompletnie nieprzewidywalne zachowania programu.	
	...
	*/
	...
	/*
		...
		*Uruchomienie takie jak <std::thread thread1 (line,'!',40);> nie gwarantuje strategii startu w¹tku.	[Strategia startu/uruchomienia w¹tku to np. fakt, ¿e w¹tek mo¿e nie wystartowaæ od razu, ale z opóŸnieniem. Mo¿na go wymusiæ, lub po utworzeniu uruchomiæ w innym momencie itp. itd.]
